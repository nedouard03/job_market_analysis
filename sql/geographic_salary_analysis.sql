...
Date: 7/14/2026
Description: Geographic Salary Analysis Queries*/
...
ranked_titles AS (
  SELECT
    state,
    CASE
    WHEN LOWER(title) LIKE '%revenue cycle%' THEN 'Revenue Cycle'
    WHEN LOWER(title) LIKE '%care management%' OR LOWER(title) LIKE '%case management%' THEN 'Care Management'
    WHEN LOWER(title) LIKE '%clinical%' THEN 'Clinical Operations'
    WHEN LOWER(title) LIKE '%health information%' OR LOWER(title) LIKE '%medical records%' THEN 'Health Information'
    WHEN LOWER(title) LIKE '%population health%' THEN 'Population Health'
    WHEN LOWER(title) LIKE '%prior auth%' OR LOWER(title) LIKE '%utilization%' THEN 'Utilization Management'
    WHEN LOWER(title) LIKE '%billing%' OR LOWER(title) LIKE '%coding%' THEN 'Billing and Coding'
    WHEN LOWER(title) LIKE '%data%' OR LOWER(title) LIKE '%analytics%' THEN 'Analytics'
    WHEN LOWER(title) LIKE '%compliance%' THEN 'Compliance'
    ELSE role_category
  END AS title_group,
...
SELECT
  role_category,
  COUNT(*) AS posting_count,
  SUM(CASE WHEN salary_available = TRUE 
    AND max_annual_salary BETWEEN 30000 AND 500000
    AND min_annual_salary BETWEEN 30000 AND 500000
    THEN 1 ELSE 0 END) AS count_with_salary_data,
  ROUND(
    (AVG(CASE WHEN salary_available = TRUE 
        AND max_annual_salary BETWEEN 30000 AND 500000 
        AND min_annual_salary BETWEEN 30000 AND 500000 
        THEN min_annual_salary END) +
     AVG(CASE WHEN salary_available = TRUE 
        AND max_annual_salary BETWEEN 30000 AND 500000 
        AND min_annual_salary BETWEEN 30000 AND 500000 
        THEN max_annual_salary END)) / 2, 2) AS avg_mid_annual_salary,
